...
function Page() {
  ...
  const HandleClick = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (Reg.length === 10) {
      SetLoader(true);
      try {
        const resp = await axios.get(`http://localhost:3000/getUserInfo/${Reg.toUpperCase()}`);
        if (resp.data !== "new user") {
          navigate(`/student_details/${Reg}`, { state: { AboutUserType: "old" } }); 
          localStorage.setItem("studentDetails", JSON.stringify(resp.data));
        } else {
          navigate(`/student_details/${Reg}`, { state: { AboutUserType: "new" } }); 
        }
        SetLoader(false);
      } catch (error) {
        console.error(error);
    ...
    <>
      <div className="Parent-container-Page w-[100%] h-[100vh] flex justify-center items-center bg-[#f2f5ff]">
        ...
        </form>
        {
          Loader && 
          <button
            className="border-2 loader_btn bg-blue-400 text-white p-2 flex justify-center"
